...
def validate_user(userid, input_password):
    ...
    if records == None:
        ...
        role = records[8]
        db_hashed_password = records[5].encode('utf-8')
        ...
        if userid == db_userid:
            ...
            if check_password_hash(input_password, db_hashed_password, salt) == True:
...
def verify(userid, password):
        input_password = userid + password
        if check_password_hash(input_password, mgr_hash, mgr_salt) == True:
